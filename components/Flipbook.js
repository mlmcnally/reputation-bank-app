...
export default function Flipbook() {
  ...
  useEffect(() => {
    ...
    // ✅ Correct destructuring for the subscription
  ...
  return (
    <div style={{ textAlign: 'center', padding: '1rem', fontFamily: 'Avenir, "Nunito Sans", sans-serif', color: '#000' }}>
      <img
        src={imageUrl}
        alt={`Page ${page}`}
        style={{ maxHeight: '88vh', width: 'auto', height: 'auto', maxWidth: '100%', objectFit: 'contain', borderRadius: '12px', border: '1px solid #ccc' }}
      />
      ...
      <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <button onClick={handleSignOut} style={{ ...baseBtn, backgroundColor: '#2ea1a8', marginTop: '0.5rem' }}>
          🔒 Sign Out
        </button>
      </div>
    </div>
